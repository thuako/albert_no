...
def get_title_and_time(areacode, theatercode, date, screencodes=None):  # ADD screen codes

    date_url = get_url(areacode, theatercode, date=date, screencodes=screencodes)  # ADD screencodes
    date_soup = get_soup(date_url)
    ...
    for coltime in coltimes:
        ...
        for typehall in typehalls:
            ...
            raw_df.append([date, title_str, theatercode, halltype, timetable])  # TBD ADD theatercode

    return pd.DataFrame(raw_df, columns=["date", "title", "theater_code", "hall_type", "timetable"])  # TBD ADD theatercode
